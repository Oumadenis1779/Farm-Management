...
def login():
    email = request.json.get('email', None)  # Get email from the request
    password = request.json.get('password', None)
    # Query user by email
    user = User.query.filter_by(email=email).first()

    if user and user.check_password(password):
        # Create access token using email as identity, or use a unique identifier
        access_token = create_access_token(identity=email)
        # Return user details including first name, last name, role, and email
        return jsonify({
